...
class PeepSoVideos
{
    ...
    public static function audio_enabled() {
        return (1==PeepSo::get_option('videos_audio_master_switch',1));
    }
    ...
    public static function get_cover_art($artist, $album, $json = FALSE) {
        ...
        $trans = 'peepso_cover_'.md5($artist.$album);
        $cache = FALSE;
        $default = TRUE;
        ...
        new PeepSoError('Cover Art: ' . print_r($response, TRUE));
    ...
    public function init()
    {
        ...
        if (is_admin()) {
            ...
            add_filter('peepso_post_types', array(&$this, 'post_types'), 20, 2);
            ...
            add_filter('peepso_postbox_tabs', array(&$this, 'postbox_tabs_audio'));
    ...
    public function admin_config_tabs($tabs)
    {
        $tabs['videos'] = array(
            ...
            'icon' => 'https://www.peepso.com/wp-content/plugins/peepso.com-checkout/assets/icons/videos_icon.svg',
            'tab' => 'videos',
    ...
    {
        wp_enqueue_script('peepsovideos', plugin_dir_url(__FILE__) . 'assets/js/bundle.min.js',
    ...
    public function postbox_tabs_audio($tabs)
    {
        ...
        if(!apply_filters('peepso_permissions_audio_upload', TRUE)) {
            return $tabs;
        }
    ...
    public function after_add_post($post_id)
    {
        ...
        if (isset($media)) {
            ...
            $enable_aws_s3 = PeepSo::get_option('videos_enable_aws_s3_elastic_transcoder');
            if ($enable_aws_s3) {
    ...
    public function parse_oembed_url($url, $accepted_type = 'video')
    {
        ...
        $media['target'] = PeepSo::get_option('site_activity_open_links_in_new_tab', 1);
    ...
    public function attach_video($post)
    {
        ...
        foreach ($post_videos as $post_video) {
            $video = array(
                ...
                'target' => PeepSo::get_option('site_activity_open_links_in_new_tab', 1),
            ...
            // Handle self-hosted video.
                ...
                // Video thumbnail.
                if ($video['thumbnail']) {
                    $html = '<div class="ps-video-thumbnail">'
                    ...
                    $html = '<div class="ps-video-thumbnail" style="background:black; padding-top:56%">'
                          . '<i class="ps-icon-youtube-play ps-video-play"></i>'
                          . '</div>';
                }

                $video['content'] = '<div class="ps-media-iframe ps-js-video">'
                    . $html
                    . '<script type="text/template">' .  $video['content'] . '</script>'
                    .'</div>';
            }
    ...
    public function activity_stream_action($action, $post)
    {
        if (self::MODULE_ID === intval($post->act_module_id)) {
            ...
            $icon = apply_filters('peepso_filter_video_action_icon','ps-icon-youtube-play');
            ...
            if ($media_type == self::ATTACHMENT_TYPE_AUDIO) {
                ...
                $icon = apply_filters('peepso_filter_audio_action_icon','ps-icon-music');
            ...
            if (strlen($title)) {
                $action = $text . '<i class="ps-post__subtitle-icon ' . $icon . '" /><i class="ps-post__subtitle-media">"' . $title . '"</i>';
            }
