...
(function(factory) {
	...
	PsVideos.prototype.init = function() {
		...
		jQuery(this.$postbox.$posttabs).on('peepso_posttabs_show-videos', function(e, tab, posttabs) {
		...
		this.$postbox.$posttabs.on('peepso_posttabs_submit-videos', function() {
			...
			if (!UPLOAD_ID && !that.has_valid_url) {
				if (null !== that.last_error) {
					psmessage.show('', that.last_error).fade_out(psmessage.fade_time);
				}
				return;
			}
			observer.addFilter(
	...
	PsVideos.prototype.on_url_input = function(url) {
		var $loading = this.$postbox.find('.ps-postbox-videos .ps-postbox-input .ps-postbox-loading'),
			...
			$.proxy(function(response) {
				...
				if (response.success) {
					...
					if (response.errors) {
						...
						psmessage.show('', this.last_error);
					}
				}
	...
	PsVideos.prototype.on_cancel = function() {
		if (this.preview_request) {
			this.preview_request.ret.abort();
			jQuery('.ps-postbox-videos .ps-postbox-input .ps-postbox-loading', this.$postbox).hide();
		}
	...
	PsVideos.prototype.option_savepost = function(post_id) {
		...
		if ($ai.length > 0) {
			...
			if (
				...
				jQuery('.cstream-edit textarea, .cstream-edit input', $ai).attr('disabled', 'disabled');
				...
				peepso.postJson('videosajax.get_preview', { url: req.url }, function(response) {
					if ('undefined' !== typeof response.errors) {
						...
						jQuery('.cstream-edit textarea, .cstream-edit input', $ai).removeAttr('disabled');
			...
			this.$file.on('change', function(e) {
				...
				that
					...
					.fail(function(errors) {
						psmessage.show('', errors[0]).fade_out(psmessage.fade_time);
					});
			});
		}
