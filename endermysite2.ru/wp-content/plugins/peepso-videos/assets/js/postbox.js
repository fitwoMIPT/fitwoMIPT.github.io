...
import peepso, { browser, observer } from 'peepso';
...
export default class PostboxVideo {
	constructor($postbox) {
		...
		this.$embedUrl.on('input', _.debounce(e => this.embed(e.target.value), 2000));
	...
	uploadInit() {
		if (!this.$uploadFile) {
			...
			this.$uploadFile.psFileupload({
				...
				add: e => {
					let file = e.target,
						$progressbar = this.$uploadProgress.find('.ps-videos__postbox-progressbar'),
					...
					this.video
						...
						.catch((data, error) => {
							let $failed = this.$uploadFailed,
								$failedMessage = $failed.find('.ps-js-failed-message');

							error = (data.errors && data.errors.join(' ')) || 'Error.';
