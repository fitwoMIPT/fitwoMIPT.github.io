...
class PeepSoVideosUpload
{
                    ...
                    if (isset($job['Job'])) {
                        ...
                        if ($status === 'Complete') {
                            ...
                            if (isset($job['Job']['Output']['ThumbnailPattern'])) {
                                ...
                                $headers = get_headers($poster_url);
	...
	public function notify_owner($post_id, $post_author, $success=TRUE)
	{
        ...
        if ($success) {
            ...
            $notification_message = __('Your video is ready', 'vidso');
            PeepSoMailQueue::add_message($post_author, $data, $notification_message, 'video_conversion_complete', 'video_conversion_complete', PeepSoVideos::MODULE_ID);
            $note->add_notification($post_author, $post_author, $notification_message, 'video_conversion_complete', PeepSoVideos::MODULE_ID, $post_id);
        } else {
            $data = $user->get_template_fields('user');

            $notification_message = __('Your video is failed to convert', 'vidso');
            PeepSoMailQueue::add_message($post_author, $data, $notification_message, 'video_conversion_failed', 'video_conversion_failed', PeepSoVideos::MODULE_ID);
            $note->add_notification($post_author, $post_author, $notification_message, 'video_conversion_failed', PeepSoVideos::MODULE_ID, $post_id);
        }
	}
