...
class PeepSoAdminVideosQueueListTable extends PeepSoListTable 
{
	...
	private function format_size_units($bytes)
    {
        if ($bytes >= 1073741824)
        {
            $bytes = number_format($bytes / 1073741824, 2) . __(' GB', 'vidso');
        }
        elseif ($bytes >= 1048576)
        {
            $bytes = number_format($bytes / 1048576, 2) . __(' MB', 'vidso');
        }
        elseif ($bytes >= 1024)
        {
            $bytes = number_format($bytes / 1024, 2) . __(' KB', 'vidso');
        }
        elseif ($bytes > 1)
        {
            $bytes = $bytes . __(' bytes', 'vidso');
        }
        elseif ($bytes == 1)
        {
            $bytes = $bytes . __(' byte', 'vidso');
        }
        else
        {
            $bytes = __('0 bytes', 'vidso');
        }
