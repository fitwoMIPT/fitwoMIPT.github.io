...
peepso.npm.objectAssign( Album.prototype, /** @lends Album.prototype */ {
	/**
	...
	edit_name: function( album_id, owner_id, elem ) {
		...
		// handle text input
		$editor.on('input', 'input', _.throttle(function( e ) {
	...
	save_name: function( album_id, owner_id, name, elem ) {
		...
		peepso.postJson( 'photosajax.set_album_name', {
			album_id: album_id,
			user_id: owner_id,
			name: name,
	...
	edit_desc: function( album_id, user_id, owner_id, elem ) {
		...
		// handle save button
		$editor.on('click', '.ps-js-submit', $.proxy(function( e ) {
			this.save_desc( album_id, user_id, owner_id, $textarea.val(), e.currentTarget );
		}, this ));
	},
	...
	save_desc: function( album_id, user_id, owner_id, description, elem ) {
		...
		peepso.postJson( 'photosajax.set_album_description', {
			album_id: album_id,
			user_id: user_id,
			owner_id: owner_id,
	...
	change_acc: function( album_id, owner_id, acc, elem ) {
		peepso.postJson( 'photosajax.set_album_access', {
			album_id: album_id,
			user_id: owner_id,
			acc: acc,
