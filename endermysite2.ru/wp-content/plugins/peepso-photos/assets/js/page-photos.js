(function($, factory) {
	...
	peepso.npm.objectAssign(PsPagePhotos.prototype, {
		init_page: function() {
			...
			this._search_$sortby = $('.ps-js-photos-sortby').on('change', $.proxy(this._filter, this));
		...
		_search_render_html: function(data) {
			_.defer(
				$.proxy(function() {
					this._search_$ct.find('.ps-js-beforeloaded').toggleClass('ps-js-beforeloaded loaded');
				}, this)
			);
		...
		_search_get_items: function() {
			return this._search_$ct.children('.ps-photos__item-wrapper');
		},
