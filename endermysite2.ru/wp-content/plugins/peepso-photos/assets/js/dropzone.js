	...
	peepso.npm.objectAssign(
		...
		/** @lends PhotoDropzone.prototype */ {
			...
			uploadInit: function() {
				...
				this.$file.psFileupload({
					...
					submit: $.proxy(function(e, data) {
						...
						data.formData = peepso.observer.applyFilters('photos_upload_req', req, this);
					}, this),
			...
			uploadAdded: function(data, id) {
				var $preview = $(this.templatePreview);
				$preview.find('.ps-js-progress').css({ width: 1 });
				$preview.on('click', '.ps-js-remove', { id: id }, $.proxy(this.onRemove, this));
				$preview.addClass('ps-js-preview-' + id);
				this.$upload.before($preview);
			...
			uploadDone: function(e, data) {
				...
				if (response.success) {
					...
					$preview.find('.ps-js-img').html('<img src="' + response.data.thumbs[0] + '" />');
