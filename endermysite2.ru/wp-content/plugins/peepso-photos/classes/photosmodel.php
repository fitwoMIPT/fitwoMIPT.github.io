...
class PeepSoPhotosModel
{
    ...
    private  $thumb_settings = array(
        's_s' => array( 128, 128, TRUE ),
        'm_s' => array( 256, 256, TRUE ),
        #'l_s' => array( 512, 512, TRUE ),
	...
	public function save_images(Array $files, $post_id, $act_id, $album_id = 0)
	{
		...
		if (NULL !== $post_activity) {
			...
			new PeepSoError('No activity record for post with ID ' . $post_id, TRUE);
		...
		foreach ($files as $file) {
			...
			if (is_file($gif_file)) {
				...
				copy($gif_file, str_replace('.jpg', '.gif', $dest_file));
				unlink($gif_file);
			}
...
//			}
	...
	private function get_amazon_s3()
	{
		...
		if (empty($aws_access_key_id) || empty($aws_secret_access_key) || empty($aws_s3_bucket)) {
			...
			new PeepSoError($this->_aws_error);
			return (NULL);
		}
    ...
    public function get_post_photos($post_id, $order='asc')
    {
        ...
        $sql .= " ORDER BY `" . $wpdb->prefix . self::TABLE . "`.`pho_created` " . $order;
    ...
    public function get_community_photos($offset = 0, $limit = 10, $sort = 'desc')
    {
        ...
        $clauses['join'] .=
            " LEFT JOIN `{$wpdb->prefix}" . self::TABLE . "` `pho` ON `pho`.`pho_post_id`=`{$wpdb->posts}`.`ID`";


		$clauses['where'] .=
				" WHERE `act_module_id`=".PeepSoSharePhotos::MODULE_ID;

		$clauses['where'] .=
				" AND `{$wpdb->posts}`.`post_status`='publish'";

		$clauses['where'] .=
				" AND `pho`.`pho_id` IS NOT NULL";
        ...
        $sql .= " GROUP BY `pho`.`pho_id` ORDER BY `pho`.`pho_created` {$sort} LIMIT {$offset}, {$limit}";
    ...
    public function get_user_photos($user_id, $offset = 0, $limit = 10, $sort = 'desc', $module_id=0)
    {
        ...
        if($module_id ==0) {
        	...
        	#var_dump($wpdb->prepare($sql, $module_id));
	...
	public function delete_content($post_id)
	{
		...
		foreach ($photos as $photo) {
			...
			if (!empty($photo->pho_token)) {
                $files[]    = $photo->pho_token;
                $files      = array_merge($files, array_values($photo->pho_thumbs));
            }
		}
		if ($enable_aws_s3 && 0 !== count($files)) {
