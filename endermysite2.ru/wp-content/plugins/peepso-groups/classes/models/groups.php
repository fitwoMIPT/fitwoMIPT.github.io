...
class PeepSoGroups
{
	...
	public function get_groups($offset, $limit, $order_by, $order, $search, $user_id = 0, $category = 0)
	{
		global $wpdb;
		...
		if(  'meta' == substr($order_by, 0, 4)) {
			$args['meta_key'] = str_replace('meta_','peepso_group_', $order_by);
			$args['orderby'] = 'meta_value_num';
			$this->where_clauses = " AND {$wpdb->prefix}postmeta.meta_key='{$args['meta_key']}'";
		}

		add_filter('posts_clauses_request', array(&$this, 'filter_post_clauses'), 10, 2);
		$post_query = new WP_Query($args);
		new PeepSoError('args : ' . json_encode($args), 'debug', 'group');
		new PeepSoError('query : ' . $post_query->request, 'debug', 'group');
		...
		while ($post_query->have_posts()) {
			...
			if($post->post_type != PeepSoGroup::POST_TYPE) { 
				new PeepSoError('PeepSoGroups::get_groups() encountered a wrong post_type', 'message','groups');
				continue; 
			}
