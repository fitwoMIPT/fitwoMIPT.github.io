...
?>
<div class="ps-focus ps-focus--group <?php if(strlen($group_segment) && 0 == PeepSo::get_option('always_full_cover', 0)) { echo 'ps-focus-mini'; } ?> js-focus ps-js-focus ps-js-group-header">
	<div class="ps-focus-cover js-focus-cover">
		<div class="ps-focus-image">
			<img id="<?php echo $PeepSoGroup->get('id'); ?>"
				...
				src="<?php echo $PeepSoGroup->get_cover_url(); ?>"
				alt="<?php echo $PeepSoGroup->get('name'); ?> cover photo"
				style="<?php echo $PeepSoGroup->cover_photo_position(); ?>"
			/>
		</div>

		<div class="ps-focus-image-mobile" style="background:url(<?php echo $PeepSoGroup->get_cover_url(); ?>) no-repeat center center;">
		</div>
		...
		<div class="ps-focus-change js-focus-change-cover" data-cover-type="cover">
			<div class="reposition-cover-actions" style="display: none;">
				<a href="#" class="ps-btn" onclick="ps_group.cancel_reposition_cover(); return false;"><?php echo __('Cancel', 'groupso'); ?></a>
				<a href="#" class="ps-btn ps-btn-primary" onclick="ps_group.save_reposition_cover(); return false;"><?php echo __('Save', 'groupso'); ?></a>
			</div>
		...
		<div class="ps-focus-header js-focus-content">
			...
			<div class="ps-focus-title ps-focus__box-toggle ps-tooltip ps-tooltio--focus-box ps-js-focus-box-toggle"
					data-tooltip="<?php echo __('Click to show more group details..', 'groupso'); ?>">
				<span>
					<?php echo $group->name; ?>
				</span>
			...
			<div class="ps-focus__box ps-js-focus-box">
				...
				<div class="ps-focus__box-details" aria-label="<?php echo __('Group details', 'groupso'); ?>">
					<?php if(PeepSo::get_option('groups_categories_enabled', FALSE)) { ?>
					<div class="ps-focus__box-details-item ps-list--separate">
						<?php if(count($group_categories) > 1) { ?><i class="ps-icon-tags"></i> <?php echo __('Group categories', 'groupso'); ?>:<?php } else { ?><i class="ps-icon-tag"></i> <?php echo __('Group category', 'groupso'); ?>:<?php } ?>
						<?php
					...
					<div class="ps-focus__box-details-item">
						<a href="<?php echo $group->get_url() . 'members/pending'; ?>">
							<span class="ps-js-member-count"><?php echo sprintf(__('(%d pending)','(%d)', 'groupso'), $group->pending_admin_members_count); ?></span>
						</a>
					</div>
					<?php } ?>
				</div>
