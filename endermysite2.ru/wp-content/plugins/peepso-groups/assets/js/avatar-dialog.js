( function( root, factory ) {
	var moduleName = 'PsGroupAvatarDialog';
	var moduleObject = factory( moduleName, root.jQuery, root.PsGroupAvatar );
	...
	var avatarDialog = null;
	var showAvatarDialog = function() {
		if ( root.peepsogroupsdata && root.peepsogroupsdata.id ) {
			if ( ! avatarDialog ) {
				var data = {
					id: +root.peepsogroupsdata.id,
					name: root.peepsogroupsdata.name,
					hasAvatar: +root.peepsogroupsdata.hasAvatar,
					imgAvatar: root.peepsogroupsdata.imgAvatar,
					imgOriginal: root.peepsogroupsdata.imgOriginal
				};
				if ( ! data.hasAvatar ) {
					data.imgOriginal = '';
				}
				avatarDialog = new PsGroupAvatarDialog( data );
			}
			avatarDialog.show();
		}
	};
	peepso.observer.addAction( 'avatar_update_dialog', showAvatarDialog );
} )( window, function( moduleName, $, PsGroupAvatar ) {
	return peepso.createClass( moduleName, {
		 ...
		 * @type {String}
		 */
		template: peepsodata.groupAvatar.templateDialog,

		/**
		 * Initialize avatar dialog.
		 * @return {PsAvatarGroupDialog}
		 */
		init: function() {
			var data, html;
			...
			if ( ! this.$el ) {
				...
				this.avatar = new PsGroupAvatar( this.$file );
				this.avatar
					...
					.on(
						'uploaddone',
						$.proxy( function( imgAvatar, imgOriginal ) {
		...
		cropConfirm: function() {
			...
			this.disable();
			this.avatar
				.crop( coords.x1, coords.y1, coords.x2, coords.y2, coords.width, coords.height )
				.done(
					$.proxy( function( imgAvatar ) {
						this.cropDetach();
