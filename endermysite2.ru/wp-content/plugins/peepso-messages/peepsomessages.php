...
class PeepSoMessagesPlugin
{
	...
	public function filter_peepso_navigation($navigation)
	{
		...
		$received = array(
			...
			'class' => 'ps-js-messages-notification',
			'title' => __('New Messages', 'msgso'),
	...
	public function enqueue_scripts()
	{
		...
		wp_register_style('msgso-selectize-default', plugin_dir_url(__FILE__) . 'assets/css/selectize.default.min.css', array(), self::PLUGIN_VERSION);
		wp_enqueue_style('msgso-selectize', plugin_dir_url(__FILE__) . 'assets/css/selectize.custom.css', array('msgso-selectize-default'), self::PLUGIN_VERSION);
		wp_register_script('msgso-selectize', plugin_dir_url(__FILE__) . 'assets/js/selectize.min.js', array('jquery'), self::PLUGIN_VERSION, TRUE);

		// Register jquery.mousewheel.js
		wp_register_script('msgso-mousewheel', plugin_dir_url(__FILE__) . 'assets/js/jquery.mousewheel.min.js', array('jquery'), self::PLUGIN_VERSION, TRUE);

		// Enqueueu main script.
		// TODO: Should be loaded on-demand!
		wp_enqueue_script('msgso', plugin_dir_url(__FILE__) . 'assets/js/peepsomessages.min.js',
		...
		if (!is_admin() && PeepSoChatModel::chat_enabled(get_current_user_id())) {

			$data_window = array(
				'read_notification' => PeepSo::get_option('messages_read_notification')
			);

			wp_register_script('peepso-chat',
			...
			wp_localize_script('peepso-chat', 'peepsochatdata', array(
				...
				'messageUrl' => PeepSo::get_page('messages') . '?view/{id}/',
				'translations' => array(
	...
	public function get_available_recipients($message_id = 0, $keyword = '', $page = 1, $user_id = FALSE)
	{
		...
		$users = new PeepSoUserSearch($args, $current_user);
