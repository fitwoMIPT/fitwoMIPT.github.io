...
class PeepSoMessagesModel
{
	 ...
	 */
	public function get_messages($type = 'inbox', $user_id, $limit = 40, $offset = 0, $query = NULL)
	{
		...
		$this->_limit = $limit;
		$this->_offset = $offset;
		$this->_query = stripslashes($query);
	...
	public function create_new_conversation($creator_id, $message, $title = '', $participants = array(), $message_data = array())
	{
		...
		if (2 === count($participants) &&	// First, check if only two participants before doing the rest of the checks
			...
			foreach ($participants as $participant_user_id) {
				...
				$trans = "msgso_notif_{$participant_user_id}_{$_msg_id}";
				set_transient($trans,1, 24*3600);
				...
				PeepSoMailQueue::add_notification($participant_user_id, $data, __('You Received a New Message', 'msgso'), 'new_message', 'new_message', PeepSoMessagesPlugin::MODULE_ID);
			}
		...
		if ($message_id) {
			...
			foreach ($participants as $participant_user_id) {
				...
				$trans = "msgso_notif_{$participant_user_id}_{$message_id}";
				set_transient($trans,1, 24*3600);
	...
	public function add_to_conversation($author_id, $parent_id, $message)
	{
		...
		if ($message_id) {
			...
			foreach ($participants as $participant_user_id) {
				...
				PeepSoMailQueue::add_notification($participant_user_id, $data, __('You Received a New Message', 'msgso'), 'new_message', 'new_message', PeepSoMessagesPlugin::MODULE_ID,0,0,1);

				// if this user was recently notified, skip
				$trans_recent = "msgso_notif_{$participant_user_id}_{$parent_id}";
				if (get_transient($trans_recent)) {
					continue;
				}



				set_transient($trans_recent,1, 24*3600);
				...
				PeepSoMailQueue::add_notification($participant_user_id, $data, __('You Received a New Message', 'msgso'), 'new_message', 'new_message', PeepSoMessagesPlugin::MODULE_ID,0,1,0);
			}
