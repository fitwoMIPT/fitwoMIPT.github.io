...
class PeepSoFriendsShortcode
{
	...
	{
		wp_enqueue_script('peepso-friends-shortcode');
	}
	...
	public function shortcode_friends()
	{
		...
		$peepso_friends = $peepso_friends->model;
		...
		wp_enqueue_script('peepso-page-friends', plugin_dir_url(__FILE__) . '../assets/js/page-friends.min.js',
	...
	public function show_friend($friend)
	{
		...
		$peepso_friends = PeepSoFriendsPlugin::get_instance();
		$mutual_friends = count($peepso_friends->model->get_mutual_friends($to_id, $from_id));

		$is_self = ($from_id == $to_id) ? TRUE : FALSE;

		$online = '';
		if (get_transient('peepso_cache_'.$friend->get_id().'_online')) {
            $online = PeepSoTemplate::exec_template('profile', 'online', array('PeepSoUser'=>$friend), TRUE);
		}

		echo '<div class="ps-members-item-wrapper">';
		echo '<div class="ps-members-item">';
		echo '<div class="ps-members-item-avatar">',
		sprintf('<div class="ps-avatar"><a href="' . $friend->get_profileurl() . '"><img alt="%s avatar" title="%s" src="%s" class="ps-name-tips"></a></div>',
		...
		, $online , do_action('peepso_action_render_user_name_before', $friend->get_id()) , $friend->get_fullname(), do_action('peepso_action_render_user_name_after', $friend->get_id()),
		'</a>
	...
	public function request_options($from_id, $to_id, $echo = TRUE)
	{
		...
		foreach ($options as $name => $data) {
			...
			$request_options .= '<i class="ps-icon-' . $data['icon'] . '"></i><span>' . $data['label'] . '</span>' . PHP_EOL;
	...
	public function request_buttons($from_id, $to_id, $echo = TRUE)
	{
		...
		foreach ($buttons as $name => $data) {
			...
			$request_buttons .= ' ">';
			if (isset($data['icon']))
				$request_buttons .= '<i class="ps-icon-' . $data['icon'] . '"></i> ';
			if (isset($data['label']))
				$request_buttons .= '<span>' . $data['label'] . '</span>';

			if (isset($data['loading']))
				$request_buttons .= ' <img style="margin-left:2px;display:none" src="' . PeepSo::get_asset('images/ajax-loader.gif') .'" alt=""></span>';

			$request_buttons .= '</button>' . PHP_EOL;
		}
