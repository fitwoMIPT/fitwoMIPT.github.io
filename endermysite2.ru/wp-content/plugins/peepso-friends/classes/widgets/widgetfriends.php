...
class PeepSoWidgetFriends extends WP_Widget
{
    ...
    public function widget( $args, $instance ) {
        ...
        if (!array_key_exists('list', $instance)) {
            ...
            $trans_list_friends = 'peepso_cache_widget_friendslist_'. $instance['user_id'];
            $trans_save_date = 'peepso_cache_widget_friendslist_savedate_'. $instance['user_id'];

            // check cache
            $list_friends = get_transient($trans_list_friends);
            $save_date = get_transient($trans_save_date);

            // load if no cache, load and cache
            if(false === $list_friends || (date('Ymd') != $save_date)) {
                $list_friends = $friendsModel->get_friends($instance['user_id'], $instance['search_args']);
                set_transient( $trans_list_friends, serialize($list_friends), 3 * HOUR_IN_SECONDS );
