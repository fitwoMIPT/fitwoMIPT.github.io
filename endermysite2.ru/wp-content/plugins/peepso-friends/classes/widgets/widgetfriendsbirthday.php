...
class PeepSoWidgetFriendsbirthday extends WP_Widget
{
    ...
    public function widget( $args, $instance ) {
        ...
        if(1 === $instance['show_upcoming_birthday']) {
            ...
            $trans_upcoming_birthday = 'peepso_cache_widget_friendsupcomingbirthday_'. $instance['user_id'];
            $trans_save_date = 'peepso_cache_widget_friendsupcomingbirthday_savedate_'. $instance['user_id'];

            // check cache
            $list_upcoming_birthday = get_transient($trans_upcoming_birthday);
            $save_date = get_transient($trans_save_date);

            // load if no cache, load and cache
            if(false === $list_upcoming_birthday || (date('Ymd') != $save_date)) {
                $list_upcoming_birthday = $friendsBirthdayModel->get_upcoming_birthday($instance['user_id'], $instance['search_args']); 
                set_transient( $trans_upcoming_birthday, serialize($list_upcoming_birthday), 3 * HOUR_IN_SECONDS );
                set_transient( $trans_save_date, date('Ymd'), 3 * HOUR_IN_SECONDS);
            } else {
        ...
        $trans_birthday = 'peepso_cache_widget_friendsbirthday_'. $instance['user_id'];
        $trans_save_date_birthday = 'peepso_cache_widget_friendsbirthday_savedate_'. $instance['user_id'];
        
        // check cache
        $list_birthday = get_transient($trans_birthday);
        $save_date_birthday = get_transient($trans_save_date_birthday);
        if(false === $list_birthday || (date('Ymd') != $save_date_birthday)) {
            $list_birthday = $friendsBirthdayModel->get_today_birthday($instance['user_id'], $instance['search_args']);
            set_transient( $trans_birthday, serialize($list_birthday), 3 * HOUR_IN_SECONDS );
            set_transient( $trans_save_date_birthday, date('Ymd'), 3 * HOUR_IN_SECONDS);
        } else {
