...
function default_gecko_copyright() {
    return sprintf(__('All rights reserved © %s','gecko'),get_bloginfo('name'));
}
//  Custom URL on logo
...
if (1==$settings->get_option( 'opt_show_adminbar', 0 ) ) : show_admin_bar( true ); endif;
if (2==$settings->get_option( 'opt_show_adminbar', 0 ) && ! current_user_can( 'manage_options' ) ) : show_admin_bar( false ); endif;
if (3==$settings->get_option( 'opt_show_adminbar', 0 ) ) : show_admin_bar( false ); endif;
...
if ( ! function_exists( 'gc_post_date' ) ) {
  function gc_post_date() {
    if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
      ...
      if (get_post_meta(get_proper_ID(), 'gecko-post-update-date', true) == 1) {
        $value = 0;
      } elseif (get_post_meta(get_proper_ID(), 'gecko-post-update-date', true) == 2) {
        $value = 1;
      } else {
        $value = $settings->get_option( 'opt_hide_blog_update', 0 );
      }

      if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) && $value == 0 ) {
      ?>
        ...
        <span class="updated" datetime="<?php esc_attr(get_the_modified_date( 'c' )); ?>">
          (<?php _e('updated', 'gecko'); ?> <?php echo get_the_modified_date(); ?>)
        </span>
      <?php
