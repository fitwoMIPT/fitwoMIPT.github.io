(function($, peepso, factory) {
	...
	peepso.observer.addFilter(
		...
		function(extras, content, root) {
			if (!content && root && !root.querySelector('.ps-stream-header')) {
					...
					this.load_images(urls).always(
						$.proxy(function() {
							...
							if ($children.length > 1) {
								this._arrange_images($ct.width() - 4, $children);
							}
	...
	/**
	 * Image arranger algorithm.
	 * http://blog.vjeux.com/2012/image/image-layout-algorithm-google-plus.html
	 * @param {int} ct_width container width
	 * @param {object} $children container children
	 */
	PsPhotos.prototype._arrange_images = function(ct_width, $children) {
		var threshold = 180,
			len = $children.length,
			dimensions = [],
			from,
		...
		for (i = 0; i < len; i++) {
			dimensions[i] = $children.eq(i).data();

			divider = 0;
			for (j = i; j >= from; j--) {
				divider += dimensions[j].width / dimensions[j].height;
			}

			height = ct_width / divider;
			...
			if (height <= threshold) {
				...
				if (height > threshold) {
					style +=
						'; margin-top: -' + Math.floor((height - threshold) / 2) + 'px !important';
				}
				$children
					.eq(from)
					.nextAll()
	...
	PsPhotos.prototype.comment_attach_photo = function(elem, files) {
		...
		$elem = $(elem);
		$addons = $elem.closest('.ps-comment-reply,.ps-comment-edit').find('.ps-js-addons');
		$addon = $addons.find('.ps-js-addon-photo');

		if (!(unique_id = $addon.data('initialized'))) {
			$file = $(
				'<input type="file" name="filedata[]" accept=".gif,.jpg,.jpeg,.png,.tif,.tiff" />'
			).uniqueId();
			$div = $('<div />').append($file);
			$div.css({
				position: 'absolute',
	...
	PsPhotos.prototype.validate_photo = function(data) {
		var reImage = /\.(gif|jpg|jpeg|png|tif|tiff)$/i,
			size = 0,
			file;
		...
		for (var i = 0; i < data.files.length; i++) {
			...
			if (!reImage.test(file.name)) {
				psmessage
					.show('', peepsophotosdata.error_unsupported_format)
	...
	PsPhotos.prototype.show_image = function(src) {
		...
		if (!$popup.length) {
			...
			$popup.on('click', '.ps-lightbox-play', function(e) {
				var $btn = $(e.currentTarget),
					...
					iconPlay = 'ps-icon-play',
					iconStop = 'ps-icon-stop',
					src = $img.attr('src'),
		...
		if (rGif.test(src)) {
			...
			$object
				...
				.removeClass('ps-icon-stop')
				.addClass('ps-icon-play')
				.show();
			$popup.show();
	...
	$(document).ready(function() {
		...
		$(document).on('ps_comment_aftersave', function(e, act_id, sel) {
			var $ct = $(sel).closest('.ps-comment-reply,.ps-comment-edit'),
	...
	peepso.observer.addFilter(
		'comment_can_submit',
		function(obj) {
			var $ct = $(obj.el).closest('.ps-comment-reply,.ps-comment-edit'),
				$photo = $ct.find('.ps-js-addon-photo'),
				$img;
