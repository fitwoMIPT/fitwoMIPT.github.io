...
class PeepSoSharePhotos
{
    ...
    public function init()
	{
        ...
        add_filter('posts_clauses_request', function($clauses, $query) {
            ...
            if (
                ...
                $clauses['where'] .= $where;
                new PeepSoError($where);
                #new PeepSoError('Trying to get modal comments for photos? '.print_r($query,true));
            }
    ...
    public function admin_config_tabs($tabs)
    {
        $tabs['photos'] = array(
            ...
            'icon' => 'https://www.peepso.com/wp-content/plugins/peepso.com-checkout/assets/icons/photos_icon.svg',
            'tab' => 'photos',
    ...
    public function enqueue_scripts()
    {
        wp_register_script('peepso-photos-bundle',
            plugin_dir_url(__FILE__) . 'assets/js/bundle.min.js',
        ...
        wp_register_script('peepso-photos-dropzone', plugin_dir_url(__FILE__) . 'assets/js/dropzone.min.js',
        ...
        wp_register_script('peepso-photos-postbox', plugin_dir_url(__FILE__) . 'assets/js/postbox.min.js',
            array('peepso', 'peepso-photos-dropzone'),
            self::PLUGIN_VERSION, TRUE);

        wp_register_script('peepso-photos-grid', plugin_dir_url(__FILE__) . 'assets/js/grid.min.js',
            array('peepso'), self::PLUGIN_VERSION, TRUE);
        wp_enqueue_script('peepso-photos-grid');

        // photo
        wp_register_script('peepso-photos', plugin_dir_url(__FILE__) . 'assets/js/peepsophotos.min.js', array('peepso-fileupload', 'peepso-photos-postbox', 'peepso-photos-dropzone', 'peepso-load-image', 'peepso-postbox', 'peepso-window'), PeepSo::PLUGIN_VERSION, TRUE);
        ...
        wp_localize_script('peepso', 'peepsophotosdata', array(
            ...
            'error_unsupported_format' => __('Supported formats are: gif, jpg, jpeg, tiff, and png.', 'picso'),
            'album_id' => $album_id
        ));

        // load scripts automatically if photo widgets is enabled
        if( is_active_widget( false, false, 'peepsowidgetphotos', true ) ) {
    ...
    public function after_add_post($post_id, $act_id)
    {
        ...
        if (count($files) > 0 && 'photo' === $input->value('type','',FALSE)) {
            $this->_photos_model->save_images($files, $post_id, $act_id);
        }
    }
    ...
    {
        $this->_photos_model->delete_content($post_id);
    }
    ...
    public function attach_photos($stream_post = NULL)
    {
        ...
        $photos = $this->_photos_model->get_post_photos($stream_post->ID, $order);
        $count = count($photos);
    ...
    public function activity_stream_action($action, $post)
    {
        ...
        if (self::MODULE_ID === intval($post->act_module_id)) {
            ...
            $photo = $this->_photos_model->get_photo($post->object_index_key);
            if(NULL != $photo) {
            ...
            if($photo_type === self::POST_META_KEY_PHOTO_TYPE_AVATAR) {
                ...
                $total_photos = $this->_photos_model->count_post_photos($post->ID);
    ...
    {
        $iterator = $this->_photos_model->get_iterator();
    ...
    {
        $photo->onclick = apply_filters('peepso_photos_photo_click', 'return ps_comments.open(' . $photo->pho_id . ', \'photo\');', $photo);
    ...
    public function get_photo_group($objects, $photo_id)
    {
        ...
        $photo = $this->_photos_model->get_photo($photo_id);

        if ( $owner_id && $album_id ) { // album photo set
            $photos = $this->_photos_model->get_user_photos_by_album($owner_id, $album_id, 0, 0, 'desc', $module_id);
        } else if ( $owner_id && $module_id == 0) { // user photo set
            $photos = $this->_photos_model->get_user_photos($owner_id, 0, 0, 'desc', $module_id);
        } else { // post (activity) photo set
            $photos = $this->_photos_model->get_post_photos($photo->pho_post_id, 'desc');
        }

        if (($count = count($photos)) > 1) {
            // #3189
            // get database timestamp
            $timestamp = $this->_photos_model->get_timestamp();
            ...
            foreach ($photos as $photo) {
                ...
                if($this->_photos_model->count_post_photos($photo->pho_post_id) == 1) {
                    $new_act_id = $this->_photos_model->get_photo_activity($photo->pho_post_id);
                    if($new_act_id) {
    ...
    public function disable_hide_activity($status) {
        if(FALSE === $status) {
            $status = TRUE;
        }
    ...
    public function get_total_object_photo($total_object, $photo_id) {
        ...
        // checking batch upload
        $total_object = $this->_photos_model->count_post_photos($photo->pho_post_id);
    ...
    public function activity_get_post($post, $activity, $owner_id, $user_id)
    {
        ...
        if (is_object($activity) && self::MODULE_ID === intval($activity->act_module_id)) {
            $photo = $this->_photos_model->get_photo($activity->act_external_id);
            ...
            if (NULL !== $photo) {
                ...
                if (NULL !== $post) {
                    ...
                    $photos = $this->_photos_model->set_photos(array($photo));
    ...
    public function delete_photo($activity)
    {
        if (self::MODULE_ID === intval($activity->act_module_id)) {
            $photo = $this->_photos_model->get_photo($activity->act_external_id);
            $this->_photos_model->delete_photo($photo);

            $total_photos = $this->_photos_model->count_post_photos($photo->pho_post_id, FALSE);

            // Delete the post when there are no more photos
            if (0 === intval($total_photos)) {
                ...
                update_post_meta($photo->pho_post_id, self::POST_META_KEY_PHOTO_COUNT, intval($total_photos)-1);
            }
        }
        else if(self::MODULE_ID === intval($activity->act_comment_module_id))
        {
            $this->_photos_model->delete_photo_comment($activity);
        }
    }
    ...
    public function change_photos_privacy($activity, $act_access)
    {
        // fix "Trying to get property of non-object" errors
        if (is_object($activity) && self::MODULE_ID === intval($activity->act_module_id)) {
            $this->_photos_model->update_post_photos_privacy($activity->act_external_id, $act_access);
        }
    }
    ...
    private function peepso_album_enqueue_scripts()
    {
        ...
        wp_register_script('peepso-photos-albumdialog', plugin_dir_url(__FILE__) . 'assets/js/albumdialog.min.js', array('peepso-npm', 'peepso-photos-dropzone', 'peepso-window'), PeepSo::PLUGIN_VERSION, TRUE);
        wp_register_script('peepso-photos-albumuploaddialog', plugin_dir_url(__FILE__) . 'assets/js/albumuploaddialog.min.js', array('peepso-npm', 'peepso-photos-dropzone', 'peepso-window'), PeepSo::PLUGIN_VERSION, TRUE);
        wp_register_script('peepso-photos-album', plugin_dir_url(__FILE__) . 'assets/js/album.min.js', array('peepso-npm'), PeepSo::PLUGIN_VERSION, TRUE);
    ...
    public function post_filters($options) {
        ...
        if($photo_type === self::POST_META_KEY_PHOTO_TYPE_ALBUM &&
            ...
            $options['delete']['icon'] = 'trash';
            $options['delete']['click'] = 'return peepso.photos.delete_stream_album(' . $post->ID . ',' . $post->act_id . ');';
            $options['delete']['label'] = __('Delete Album', 'picso');
        }
    ...
    {
        if(0 !== $user_id && apply_filters('peepso_permissions_post_create', TRUE) === TRUE){
    ...
    public function after_add_post_cover($post_id, $act_id)
    {
        ...
        if(!empty($file)) {
            $this->_photos_model->save_images_profile($file, $post_id, $act_id,$album);
        }
    }
    ...
    public function after_add_comment($post_id, $act_id)
    {
        ...
        if(FALSE !== $photo)
        {
            $this->_photos_model->save_images_comment($photo, $post_id, $act_id);
        }
    }
    ...
    public function after_save_comment($post_id, $activity)
    {
        ...
        $this->_photos_model->save_images_comment($photo, $post_id, $activity->act_id);
    }
    ...
    public function commentsbox_addons($addons, $post_id = FALSE)
    {
        ...
        if ($post_id) {
            ...
            $photo_comment = $this->_photos_model->get_photo_comments($comment);
    ...
    {
        $peepso_photo_comments = $this->_photos_model->get_photo_comments($stream_comment);
    ...
    public function after_add_post_album($post_id, $act_id)
    {
        ...
        if (count($files) > 0 && 'album' === $input->value('type','',FALSE)) {
            ...
            $this->_photos_model->save_images($files, $post_id, $act_id, $album_id);
        }
    }
     ...
     * check for gif file
     */
	public function is_gif_file($location) {
		$gif_file_location = str_replace('.jpg', '.gif', $location);
