...
class PeepSoGroupsListTable extends PeepSoListTable
{
	...
	public function column_group($group)
	{
		return sprintf('<a href="%s" target="_blank">%s <i class="fa fa-external-link"></i></a><a href="%s" target="_blank"><img src="%s" class="ps-avatar"></a> <span>'
			. __('Privacy', 'groupso') . ': <i class="'.$group->privacy['icon'].'"></i> <strong>'.$group->privacy['name'] . '</strong></span>',
			$group->get_url(),
			$group->get('name'),
			$group->get_url(),
	...
	public function process_bulk_action()
	{
		if ($this->current_action() && check_admin_referer('bulk-action', 'groups-nonce')) {
				...
				foreach ($posts as $id) {
					...
					if('publish' === $this->current_action()) {
						if($this->current_action() !== $the_post->post_status){
							$notif->add_notification($user_id, $the_post->post_author, __('published your group', 'groupso'), 'groups_publish', PeepSoGroupsPlugin::MODULE_ID, $id);
						}
					} elseif('unpublish' === $this->current_action()) {
						if($this->current_action() !== $the_post->post_status){
							$notif->add_notification($user_id, $the_post->post_author, __('unpublished your group', 'groupso'), 'groups_unpublish', PeepSoGroupsPlugin::MODULE_ID, $id);
						}
					}
				}
			...
			PeepSoAdmin::get_instance()->add_notice(
				sprintf(__('%1$d %2$s %3$s', 'groupso'),
					$count,
					_n('group', 'gorups', $count, 'groupso'),
					$message),
				'note');

            do_action('peepso_action_plugin_cleanup', 'groups','basic');
