...
class PeepSoGroup
{
	...
	public static function validate($group_data)
	{
		...
		foreach($group_data as $property=>$value) {
			...
			if ( isset($rule['minlength']) && is_numeric($rule['minlength'])  && strlen($value) < $rule['minlength']) {
				$errors[$property] = sprintf( __('Minimum length is %s characters', 'groupso'), $rule['minlength'] );
			}

			if ( isset($rule['maxlength']) && is_numeric($rule['maxlength'])  && strlen($value) > $rule['maxlength'] ) {
	...
	public function get_avatar_url($full = TRUE)
	{
		...
		if (!file_exists($file)) {
			...
			$file = PeepSo::get_asset('images/avatar/group' . ($full ? '' : '-thumb') . '.png');
		} else {
	...
	public function finalize_move_avatar_file($add_to_stream=TRUE)
	{
		...
		if (file_exists($src_thumb) && file_exists($src_full) && file_exists($src_orig)) {
			...
			if($add_to_stream && class_exists('PeepSoSharePhotos')) {
    ...
    {
		$cover = PeepSo::get_asset('images/cover/group-default.png');
