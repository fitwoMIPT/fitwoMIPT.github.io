...
peepso.npm.objectAssign( PsGroupDlgCreate.prototype, /** @lends PsGroupDlgCreate.prototype */ {
	/**
	 * Dlg template.
	 * @type {string}
	 */
	...
	create: function() {
		...
		this.$el.on('click', '.ps-js-dropdown--privacy .ps-js-dropdown-menu a', $.proxy( this.onChangePrivacy, this ));
	...
	onChangePrivacy: function( e ) {
		...
		// Update label.
		$toggle.find( '.dropdown-value span' ).text(
			$a.find( '.ps-dropdown__group-title span' ).text()
		);

		// Update icon.
		$toggle.find( '.dropdown-value [class*=ps-icon]' ).attr( 'class',
	...
	onSubmit: function() {
		...
		peepso.postJson( 'groupajax.create', req, $.proxy(function( json ) {
			var errors;
			...
			if ( json.success ) {
				...
				if ( error.category_id ) {
					this.$el.find('.ps-js-error-category_id').html( error.category_id ).show();
				} else {
