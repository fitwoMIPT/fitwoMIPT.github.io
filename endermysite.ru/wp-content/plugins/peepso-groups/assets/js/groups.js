		...
		peepso.observer.addFilter(
			...
			function(group_id, member_count) {
				var $item = $('.ps-js-group-item--' + group_id),
					...
					html;
				if ($count.length) {
					label = +member_count > 1 ? peepsogroupsdata.lang.members : peepsogroupsdata.lang.member;
		...
		$(document.body).on(
			...
			$.proxy(function(e) {
				var itemSelector = '.ps-js-group-item',
					expandedClassName = 'ps-group__item--expanded',
					...
					$.proxy(function(response) {
						...
						if (data.groups && data.groups.length) {
							...
							for (i = 0; i < data.groups.length; i++) {
								...
								if (params.query) {
									reQuery = params.query.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, '\\$&');
