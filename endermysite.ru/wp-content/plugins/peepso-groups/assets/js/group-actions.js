...
export default class GroupActions {
    constructor( data ) {
        ...
        this.$el = $( '<div />' );
        this.$el.on( 'click', '[data-method]', ( e ) => { this.onClick( e ) });
    ...
    doAction( action, data ) {
        return new Promise(( resolve, reject ) => {
            peepso.postJson( action, data, json => {
                if ( json.success ) {
    ...
    onClick( e ) {
        ...
        this.confirmAction( confirm ).then(() => {
            $btn.data( 'ps-loading', true );
            $loading.show();
            this.doAction( method, data ).then( data => {
                let actions = this.constructor.normalize( data );
                this.actions = _.defaults({}, actions, this.actions );
                this.render();

                if ( data.member_count ) {
                    observer.applyFilters( 'group_update_member_count', this.id, data.member_count );
                }
            });
        }).catch( $.noop );
    }
    ...
    static normalize( data ) {
        ...
        if ( group.groupfollowerajax && typeof group.groupfollowerajax.follower_actions !== 'undefined' ) {
            follower_actions = prefixer( group.groupfollowerajax.follower_actions || [], 'groupfollowerajax' );
