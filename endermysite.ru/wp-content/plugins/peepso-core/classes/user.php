...
class PeepSoUser
{
    ...
    public function has_cover()
    {
        ...
        $arr_path = explode(DIRECTORY_SEPARATOR, $peepso_dir);
        $c = count($arr_path);

        if (FALSE !== stripos($cover, $arr_path[$c-2] . DIRECTORY_SEPARATOR . $arr_path[$c-1])) {
            return (TRUE);
        }
        return (FALSE);
    }
        ...
        if(file_exists($this->get_image_dir() . $filename)) {
            ...
            if (is_int($size) && $size > 0) {
                ...
                if (!file_exists($this->get_image_dir() . $filename_scaled)) {
                    ...
                    $si->png_to_jpeg($this->get_image_dir() . $filename);
	...
	{
        $name_parts = $this->get_name_parts(FALSE, 'real_name_first');
	...
	public static function set_stream_filters($input) {
	    if($input instanceof PeepSoInput) {
            ...
            if(PeepSo::get_option('stream_id_sticky', 1)) {
            ...
            // show my posts
            $default = 1;
            $show_my_posts = $input->value('stream_filter_show_my_posts', $default, FALSE); // SQL Safe
            if(!in_array($show_my_posts, array(0,1))) {
	...
	public static function get_stream_filters() {
	    ...
	    if($user_id = get_current_user_id()) {
            ...
            // Force revalidation
            if(!in_array($r['show_my_posts'], array("0","1"))) {
                $r['show_my_posts'] = 1;
            }
...
//		$this->profile_fields->set('gender', $gender);
	...
	public function update_user_notification() {
		if ($this->id > 0) {
			// Disable all  e-mail alerts for new member
			if(1==PeepSo::get_option('new_member_disable_all_email_notifications', 0)) {

	            $PeepSoProfile = PeepSoProfile::get_instance();
	            $PeepSoTags = PeepSoTags::get_instance();
	            $alerts = $PeepSoProfile->get_available_alerts();

	            foreach ($alerts as $alert) {
	                foreach ($alert['items'] as $alert) {
	                    $user_alerts[] = $alert['setting'] . '_email';
	                }
	            }

	            update_user_meta($this->id, 'peepso_notifications', $user_alerts);
	            update_user_meta($this->id, 'peepso_email_digest_receive_enabled', 0);
	        }
		}
	} 
