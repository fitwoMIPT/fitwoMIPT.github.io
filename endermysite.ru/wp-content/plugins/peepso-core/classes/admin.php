...
class PeepSoAdmin
{
    ...
    const TRANS_PLUGINS = 'peepso_plugins';
...
//		);
            ...
            $tabs = array(
				'red' => array(
                    ...
                    'getting-started' => array(
                        'slug' => 'peepso-getting-started',
                        'menu' => '<span style="color:#4CBBA7">'.__('Getting Started', 'peepso-core').'</span>',
                        'icon' => 'info',
                        'function' => array('PeepSoGettingStarted', 'init')
                    ),

                    'extensions' => array(
                        'slug' => 'peepso-extensions',
                        'menu' => '<span style="color:#f39c12">'.__('Addons', 'peepso-core').'</span>',
                        'icon' => 'admin-plugins',
            ...
            $show_addons = isset($_REQUEST['show_addons']) ? intval($_REQUEST['show_addons']) : PeepSo::get_option('show_addons', 1);
            if(0==$show_addons) {
                unset($tabs['red']['extensions']);
            }

            $show_getting_started = isset($_REQUEST['show_getting_started']) ? intval($_REQUEST['show_getting_started']) : PeepSo::get_option('show_addons', 1);
            if(0==$show_getting_started) {
                unset($tabs['red']['getting-started']);
            }
	...
	public function define_dashboard_metaboxes()
	{
        ...
        $this->messages = $this->system_check();

        if( count($this->messages['errors']) || count($this->messages['warnings']) ) {
            $dashboard_metaboxes[] = array(
                'name' => 'system',
                'title' => __('System Requirements', 'peepso-core'),
                'callback' => array(&$this, 'system_metabox'),
                'context' => 'right'
            );
        }
        ...
        if(0==PeepSo::get_option('bundle',0)) {
            $dashboard_metaboxes[] = array(
                'name' => 'ultimate_bundle',
                'title' => __('Ultimate Bundle', 'peepso-core'),
                'callback' => array(&$this, 'bundle_metabox'),
                'context' => 'right'
            );
        }
	...
	public function system_check()
    {
        ...
        $php_sessions = session_status();
        $php_memory = ini_get('memory_limit');
        $mysql_version = $wpdb->db_version();
        $php_imagick = (extension_loaded('imagick')) ? TRUE : FALSE;
        $php_exif = function_exists('exif_read_data') ? TRUE : FALSE;

        $req = array(
            'php_min'   => PeepSoSystemRequirements::PHP_REQUIRED,
            'php_best'  => PeepSoSystemRequirements::PHP_RECOMMENDED,
            'memory_min'=> PeepSoSystemRequirements::MEMORY_REQUIRED,
            'mysql_min' => PeepSoSystemRequirements::MYSQL_REQUIRED,
        );
        ...
        // PHP Version
        if(version_compare($php_version, $req['php_min']) < 0) {
            $messages['errors'][] = sprintf(__('The minimum required PHP version is %s - your server has %s','peepso-core'),$req['php_min'],$php_version);
        }

        elseif(version_compare($php_version, $req['php_best']) < 0) {
            $messages['warnings'][] = sprintf(__('The recommended PHP version is %s or higher - your server has %s. Please visit %s for more information.','peepso-core'),$req['php_best'], $php_version, '<a href="http://php.net/supported-versions.php" target="_blank">PHP.net</a>');
        }

        // PHP EXIF
        if (!$php_exif) {
            $messages['errors'][] = __('The PHP EXIF extension is required','peepso-core');
        }

        // PHP Imagick
        if (!$php_imagick) {
            $messages['warnings'][] = __('The PHP Imagick extension is recommended','peepso-core');
        }

        // PHP Memory limit
        if($this->return_bytes($php_memory) < $this->return_bytes($req['memory_min'])) {
            $messages['errors'][] = sprintf(__('The required memory limit is %s - your server has %s','peepso-core'), $req['memory_min'], $php_memory);
        }

        // PHP Sessions
        if(PHP_SESSION_DISABLED == $php_sessions) {
            $messages['errors'][] = __('PHP session support is required','peepso-core');
        }

        // MySQL (skip if mariaDB or non-MySQL)
        if($wpdb->is_mysql && !stristr($wpdb->dbh->server_info, 'maria') && version_compare($mysql_version, $req['mysql_min']) < 0) {
            $messages['errors'][] = sprintf(__('The minimum required MySQL version is %s - your server has %s','peepso-core'),$req['mysql_min'],$mysql_version);
        }

        // Connectivity check failed 10 times
        if(strlen(get_transient('peepso_is_offline'))) {
            $messages['warnings'][] = __('Can\'t connect to PeepSo.com. Please contact PeepSo support.', 'peepso-core');
        }
	...
	public function newsletter_metabox()
	{
		...
		<div class="row">
                ...
                if(isset($_GET['newsletter_purge'])) {
                    delete_transient('peepso_newsletter_form');
                    delete_transient('peepso_newsletter_form_new');
                }

                if(!isset($_GET['newsletter_test'])) {
                    $newsletter_form = get_transient('peepso_newsletter_form');
                    ...
                    if (!strlen($newsletter_form)) {
                        ...
                        if (is_wp_error($resp)) {
                            ...
                            set_transient('peepso_newsletter_form', $newsletter_form, 3600 * 24);
                        }
                    }
                } else {
                    $newsletter_form = get_transient('peepso_newsletter_form_new');
                    ...
                    if (!strlen($newsletter_form)) {
                        ...
                        if (is_wp_error($resp)) {
                            ...
                            $newsletter_form = $resp['body'];
                            set_transient('peepso_newsletter_form_new', $newsletter_form, 3600 * 24);
                        }
                    }
                }
    ...
    public function bundle_metabox()
    {
	...
	public static function extensions() {
		...
		$plugins = get_transient(self::TRANS_PLUGINS);
