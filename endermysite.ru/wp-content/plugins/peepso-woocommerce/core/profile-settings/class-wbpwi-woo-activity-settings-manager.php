...
if ( ! class_exists( 'WBPWI_Woo_Activity_Settings_Manager' ) ) :
	...
	class WBPWI_Woo_Activity_Settings_Manager {
		...
		public function wbpwi_peepso_preference_activity_settings( $pref ) {
			...
			if ( $purchase_setting ) {
				$purchase_setting_value = $this->wbpwi_user_meta( self::$order_setting_name, get_current_user_id() );
				$purchase_setting_value = $purchase_setting_value === 0 ? 1 : $purchase_setting_value;
				$activity_setting_fields[ self::$order_setting_name ] = array(
					...
					'value'      => $purchase_setting_value,
					'type'       => 'yesno_switch',
					'loading'    => true,
				);
			}
			if ( $review_setting ) {
				$review_setting_value = $this->wbpwi_user_meta( self::$order_setting_name, get_current_user_id() );
				$review_setting_value = $review_setting_value === 0 ? 1 : $review_setting_value;
