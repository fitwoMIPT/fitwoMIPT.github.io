	...
	peepso.npm.objectAssign(
		...
		/** @lends PsChat.prototype */ {
			...
			create: function() {
				...
				this.$wrapper = this.$root.children('.ps-chat-wrapper');
				this.$windows = this.$wrapper.children('.ps-chat-windows');
				...
				$(window)
					.off('resize.ps-chat')
					.on('resize.ps-chat', $.proxy(this.onDocumentResize, this));
							...
							if (response.success) {
								...
								for (i = 0; i < response.data.chats.length; i++) {
									...
									if (
										...
										this.state[chat.id].receipt_unread !== +chat.receipt_unread
									) {
										// set to false for sidebar on first call to fix false positive flag
										if (!this.state[chat.id].last_activity && i >= MAX_VISIBLE_WINDOWS) {
			...
			renderWindows: function() {
				...
				if (this.windowsOrder.length) {
					if (!this.backspacePrevented) {
						...
						$(document).on('keydown.ps-chat', function(e) {
