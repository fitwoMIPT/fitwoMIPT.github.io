(function($, peepso, factory) {
	ps_messages = peepso.messages = factory($, peepso);
	...
	PsMessages.prototype.init_conversation_view = function(conversation_id) {
		...
		peepso.observer.addFilter('peepso_postbox_enter_to_send', function() {
			return ps_messages.enter_to_send;
		});

		peepso.observer.addFilter('peepso_postbox_input_changed', function(val) {
		...
		peepso.observer.addFilter('psmessages_conversation_view', function() {
			return true;
		});
		...
		peepso.observer.addFilter(
			...
			function(msg_id) {
				...
				if (+msg_id === +conversation_id) {
					$btns = $('.ps-js-btn-mute-conversation');
					$btns.attr(
						'onclick',
						'return ps_messages.unmute_conversation(' + msg_id + ', 0);'
					);
					$btns.attr('title', peepsomessagesdata.unmute_conversation);
					$icons = $btns.find('i');
					$icons.attr('class', 'ps-icon-bell-off');
				}
		...
		peepso.observer.addFilter(
			...
			function(msg_id) {
				...
				if (+msg_id === +conversation_id) {
					...
					$btns.attr('title', peepsomessagesdata.mute_conversation);
					$icons = $btns.find('i');
					$icons.attr('class', 'ps-icon-bell-alt');
				}
		...
		return $.Deferred(
			$.proxy(function(defer) {
				...
				peepso
					.disableAuth()
					.disableError()
					.postJson('messagesajax.get_messages_in_conversation', req, function(response) {
						...
						if (direction === 'old') {
							setTimeout(function() {
								...
								if (
									...
									$messages = peepso.observer.applyFilters(
										'messages_render',
										$messages
									);
									peepso.observer.doAction('peepso_external_link', $messages);
									$messages.insertAfter(ps_messages.$loading);
									ps_messages.$conversation
										.stop()
							...
							if (response.success) {
								ps_messages.submit_entertosend ||
									ps_messages.toggle_entertosend(+response.data.enter_to_send);
								if (response.data.ids && response.data.ids.length) {
									...
									peepso.observer.doAction('peepso_external_link', $messages);
									$messages.insertBefore(ps_messages.$typing);
									ps_messages.$doc.trigger('peepso_messages_list_displayed');
									newChatAdded = true;
									scroll2bottom = true;
								}
						...
						if (response.data && response.data.users) {
							...
							$('.ps-conversation__status')
								.children()
								.get(0).className = online ? 'ps-icon-circle' : 'ps-icon-clock';
						}
						...
						if (response.data && 'undefined' !== typeof response.data.receipt) {
							...
							if (ps_messages.receipt !== receipt || ps_messages.unread !== unread) {
						...
						if (response.data && 'undefined' !== typeof response.data.send_receipt) {
	...
	PsMessages.prototype.smart_long_polling = function(delay) {
		...
		peepso.observer.addAction('browser.inactive', function() {
			doPolling(maxDelay);
		});

		peepso.observer.addAction('browser.active', function() {
			doPolling(minDelay);
		});
	};
	...
	PsMessages.prototype.new_message = function(user_id, flag, btn) {
		...
		peepso.observer.addAction(
			...
			(dlgBeforeClose = function($dialog) {
				peepso.observer.removeAction('pswindow_before_close', dlgBeforeClose);
				peepso.observer.doAction('psmessage_new_message_close', $dialog);
			}),
			10,
			1
		);
		...
		var $message_postbox = (this.$message_postbox = $(
			...
			on_save: function(response) {
				pswindow
					.hide()
					.show('', response.notices[0])
					.fade_out(pswindow.fade_time);

				peepso.observer.removeFilter('beforeunload', $message_postbox.beforeUnloadHandler);
	...
	PsMessages.prototype.compact_map = function($el) {
		if ($el.closest('.ps-dialog').length > 0) {
			$el.find('.ps-postbox-location').addClass('ps-postbox-location-compact');
		}
	};
	...
	PsMessages.prototype.toggle_send_button = function() {
		if (!this.$recipient_select.val()) {
			this.$message_postbox.$save_button._detached = this.$message_postbox.$save_button.parent();
	...
	PsMessages.prototype.load_messages = function(type, page, query) {
		...
		peepso.postJson('messagesajax.get_messages', req, function(response) {
			...
			if (response.success) {
				...
				$($messages, '.ps-js-messages-search-form').on('submit', function() {
					$query = $("input[name='query']", this);
	...
	PsMessages.prototype.apply_bulk_action = function(elem, type, page) {
		...
		var ckbx = $('#peepso-wrap .ps-message--inbox input:checked');

		if (ckbx.length > 0) {
			if ('' === $form.find("select[name='action']").val()) {
				psmessage.show('', $('#peepsomessages-no-action-selected').html());
			} else {
				if (false === this.ajax_req) {
					if ('delete' === $form.find("select[name='action']").val()) {
						pswindow.confirm_delete(function() {
							_self._bulk_action(data, type, page);
							pswindow.hide();
						});
					} else {
						this.ajax_req = this._bulk_action(data, type, page);
					}
				}
			}
		} else {
			psmessage.show('', $('#peepsomessages-no-item-selected').html());
		}
	};
	...
	PsMessages.prototype.delete_single_message = function(msg_id) {
		pswindow.confirm_delete(function() {
			pswindow.hide();
			peepso.postJson('messagesajax.delete_from_conversation', { msg_id: msg_id }, function(
				response
			) {
				if (response.success) {
	...
	PsMessages.prototype.update_checkmark = function(msg_id, status) {
		...
		if (status) {
			...
			$btns.attr('title', peepsomessagesdata.hide_checkmark);
			$btns.removeClass('disabled');
		} else {
			$btns.attr('onclick', 'return ps_messages.toggle_checkmark(' + msg_id + ', 1);');
			$btns.attr('title', peepsomessagesdata.show_checkmark);
	...
	PsMessages.prototype.toggle_message_checkmark = _.throttle(function(unread) {
		var $chatbox = $('.ps-chat__messages'),
			$checkmarks = $('.ps-chat__messages').find('.ps-icon-ok'),
			$unread;
	 ...
	 * @param {int} msg_id ID from to-be-muted conversation.
	 */
	 ...
	 * @param {int} msg_id ID from to-be-muted conversation.
	 * @param {int} elem Mute button inside the mute conversation confirmation dialog.
	 */
	PsMessages.prototype.confirm_mute_conversation = function(msg_id, elem) {
		...
		peepso.postJson('messagesajax.set_mute', { parent_id: msg_id, mute: value }, function(
			response
		) {
			if (response.success) {
				peepso.observer.applyFilters('psmessages_conversation_mute', msg_id);
			}
		});
	...
	PsMessages.prototype.unmute_conversation = function(msg_id) {
		peepso.postJson('messagesajax.set_mute', { parent_id: msg_id, mute: 0 }, function(
			response
		) {
			if (response.success) {
				peepso.observer.applyFilters('psmessages_conversation_unmute', msg_id);
			}
		});
		return false;
	};
	...
	$(function() {
		...
		var elem = $('.ps-js-messages-notification')
			...
			.on('notifications.shown', function(e, obj) {
				var selector = '.ps-notification__wrapper > .ps-messages-item';
