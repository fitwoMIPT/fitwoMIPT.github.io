...
peepso.npm.objectAssign( PsChatSidebar.prototype, peepso.npm.EventEmitter.prototype, /** @lends PsChatSidebar.prototype */ {
	/**
	...
	update( id, $item ) {
		...
		if ( $item.length ) {
			...
			const fetcher = () => {
				...
				this.fetchQueueExec = this.fetchQueueExec || $.proxy(function() {
	...
	formatParticipants: function( users ) {
		...
		if ( users.length === 1 ) {
			...
			str = str.join(', ');
			if ( users.length === 2 ) {
				str = this.translations.and.replace( /%s(.+)%s/, str + '$1' + users[ users.length - 1 ].name_first );
			} else if ( users.length === 3 ) {
				str = this.translations.and_x_other.replace( '%s', str ).replace( '%d', 1 );
			} else {
				str = this.translations.and_x_others.replace( '%s', str ).replace( '%d', users.length - 2 );
			}
		}
		return str;
	},
