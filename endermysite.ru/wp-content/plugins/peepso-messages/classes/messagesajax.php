...
class PeepSoMessagesAjax extends PeepSoAjaxCallback
{
	...
	public function add_message(PeepSoAjaxResponse $resp)
	{
		...
		$trans = 'msgso_typing_' . $user_id. '_' . $parent_id;
		delete_transient($trans);

		// And that another one can't be set for another full second
		$trans_just_posted = "msgso_posted__{$user_id}_{$parent_id}";
		set_transient($trans_just_posted,1,1);
	...
	public function i_am_typing(PeepSoAjaxResponse $resp)
	{
		...
		if( $msg_id > 0) {
			$trans = "msgso_typing_{$user_id}_{$msg_id}";

			$trans_just_posted = "msgso_posted__{$user_id}_{$msg_id}";
			if(!get_transient($trans_just_posted)){
	...
	public function get_messages_in_conversation(PeepSoAjaxResponse $resp)
	{
		...
		if( 1 === $get_recently_deleted) {
			$trans = 'msgso_del_' . $user_id;
			$recently_deleted = get_transient($trans);

			if (strlen($recently_deleted)) {
		...
		foreach($participants as $check_user) {
			...
			$trans = 'msgso_typing_' . $check_user . '_' . $msg_id;

			if( $is_typing = get_transient($trans)) {
			...
			foreach ($participants as $participant_user_id) {
				...
				if($participant_user_id != $user_id) {
					...
					$users[] = array(
						...
						'online' 	=> get_transient('peepso_cache_'.$user->get_id().'_online'),
	...
	public function delete_from_conversation(PeepSoAjaxResponse $resp)
	{
		...
		if( $message_recipients->delete_from_conversation($user_id, $msg_id)) {
			...
			if($recently_deleted = get_transient($trans)) {
			...
			set_transient($trans, $recently_deleted, 5*60);
	...
	public function add_participants(PeepSoAjaxResponse $resp)
	{
		...
		if(0 == $is_group) {
			...
			$url = PeepSo::get_page('messages') . '?view/'.$msg_id.'/';
	...
	public function get_messages(PeepSoAjaxResponse $resp)
	{
		...
		// perform the search
		$model->get_messages($type, $user_id, $per_page, $offset, $query);
