...
class PeepSoVideosAjax extends PeepSoAjaxCallback
{
	...
	public function get_preview(PeepSoAjaxResponse $resp)
	{
		...
		if (FALSE === $response) {
            ...
            if ($accepted_type == 'video') {
                ...
                $resp->error(__('Sorry, this is not a valid audio.<br><small>Please make sure you\'re using a valid URL, not and embed code.</small>', 'vidso'));
            }
		} else {
			$resp->success(TRUE);
    ...
    public function get_user_videos(PeepSoAjaxResponse $resp)
    {
        ...
        $media_type = 'all';
        $videos = $videos_model->get_user_videos($media_type, $owner, $offset, $limit, $sort, $module_id);
    ...
    public function get_user_videos_count(PeepSoAjaxResponse $resp)
    {
        ...
        $resp->success(1);
        $resp->set('found_videos', $videos_model->get_num_videos($media_type, $this->_input->int('user_id')));
    }
    ...
    public function validate_video_upload(PeepSoAjaxResponse $resp)
    {
        ...
        } else if (!$videos_model->video_size_can_fit($user->get_id(), $this->_input->int('size', 0))) {
            $error = __('Maximum file upload quota reached. Delete posts with videos to free some space.', 'vidso');
        }
    ...
    public function upload_video(PeepSoAjaxResponse $resp)
    {
        if (count($_FILES) > 0 && isset($_FILES['filedata'])) {
            ...
            if (!$videos_model->video_size_can_fit($user->get_id(), $this->_input->int('size', 0))) {
            ...
            $max_upload_size = PeepSo::get_option('videos_max_upload_size');
            ...
            if ($_FILES['filedata']['size'][0] >= $max_upload_size * 1048576) {
                $resp->error(sprintf(__('Only files up to %1$dMB are allowed', 'vidso'), $max_upload_size));
                $resp->success(FALSE);
                return;
            }
            ...
            if (isset($filetype['type']) && !in_array($filetype['type'], $this->allowed_mime_types)) {
            ...
            if(!file_exists($file)) {
                ...
                $is_audio = $video_upload->check_audio_by_type( $mimetype );
