...
class PeepSoFriendsRequests
{
	...
	public function add($from_id, $to_id)
	{
		if ($from_id !== $to_id) {
			...
			if ($this->request_status($from_id, $to_id)) {
                return new PeepSoError(__('Friend request already sent', 'friendso'));
            }
	...
	public function accept($request_id)
	{
		...
		if ($from_id !== $to_id) {
			...
			if ($peepso_friends->model->add_friend($from_id, $to_id)) {
				global $wpdb;
	...
	public function show_request_thumb($request = NULL)
	{
		...
		$mutual_friends = count($peepso_friends->model->get_mutual_friends(get_current_user_id(), $user_id));

		echo '<div class="ps-members-item-avatar">
				<div class="ps-avatar">',
			sprintf('<img alt="%s" title="%s" src="%s" class="ps-name-tips">',
				$friend->get_fullname(),
				$friend->get_fullname(),
				$friend->get_avatar()
			),
	...
	public function after_add($from_id, $to_id)
	{
		...
		PeepSoMailQueue::add_notification($to_id, $data, $title, 'friend_request_send', 'friends_requests', PeepSoFriendsPlugin::MODULE_ID);
	}
	...
	public function after_accept($from_id, $to_id)
	{
		...
		PeepSoMailQueue::add_notification($from_id, $data, $title, 'friend_request_accept', 'friends_requests', PeepSoFriendsPlugin::MODULE_ID);

		$note = new PeepSoNotifications();
		$note->add_notification($to_id, $from_id, __('accepted your friend request', 'friendso'), 'friends_requests', PeepSoFriendsPlugin::MODULE_ID);
	}
